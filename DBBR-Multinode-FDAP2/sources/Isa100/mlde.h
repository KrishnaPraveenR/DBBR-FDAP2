...
#define DLL_ADV_OPT_ELEDE_JOIN          0x04
...
extern WCITXT_TX_MSG g_stWciRcvRfMsg;
extern WCITXT_TX_MSG g_stWciRcvAckMsg;
extern WCITXT_TX_MSG g_stWciTxRfMsg;

/* Tx ACK PDU item */
#ifdef CC2420_AES_H    
  #define MAX_ACK_LEN                     26      //2 bytes MHR Frame control 
                                                  //1 byte Sequence Number
                                                  //2 bytes Subnet ID
                                                  //8 bytes source EUI64 Address(when requested)
                                                  //1 byte DHR Frame control
                                                  //4 bytes echoed MIC(even if the MIC is on 64 bits)
                                                  //2 bytes time correction(when requested)
                                                  //2 bytes slow hopping timeslot offset(when needed)
                                                  //4 bytes DAUX header(when requested)
#else
...
#define DLL_MASK_DL_VERSION_DPDU     0x01   //DHDR frame control related to DPDU (ISA 2011)
#define DLL_MASK_DL_VERSION_ACK      0x03   //DHDR frame control related to Ack (ISA 2011) 

                   ...
                   uint8*  p_pRspBuf)
{
  ...
  uint16 unDummyRspSize = 0;
  
  switch(p_ucMethID)
  {      
      case DLMO_WRITE_ROW_NOW:
      case DLMO_WRITE_ROW:  ucSFC = DLMO_WriteRow(unAttrID, ulSchedTAI, p_unReqSize, p_pReqBuf, &unDummyRspSize); break;
                   ...
                   uint8*   p_pRspBuf)
{
  ...
  #warning "Get SMIB functions should check if rsp buffer is large enough, to prevent memory corruption. Anyway, MAX_RSP_SIZE is 220 and it should be enough"
                    ...
                    uint16* p_pRspSize)
{
  ...
  *p_pRspSize = p_unReqSize;  // p_pRspSize is in/out parameter in the following function
    
  // now start parsing the SMIB    
  LOCAL_SMIB_PARSER_IDX ucLocalParserIdx = (LOCAL_SMIB_PARSER_IDX)(c_ucAttrParserInfo[p_unAttrID].m_ucLocalParserIdx & 0x7F);
    
  ucRetSFC = c_aSetSMIBFct[ucLocalParserIdx](p_pRspSize, p_pReqBuf, &stTwoIdxSMIB.m_stEntry);  
  
  if(SFC_SUCCESS != ucRetSFC)
  {
      #warning "To decide if rspSize (number of parsed octets is needed by caller for errors!"
      *p_pRspSize = 0;
      return ucRetSFC;
  }

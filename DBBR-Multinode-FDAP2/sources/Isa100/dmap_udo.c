...
#include "aslsrvc.h"
#include "mlsm.h"
#include "../bootl.h"  //for DEV_TYPE_MC13225
...
const uint16 c_unMaxBlockSize = 98;//MAX_DLL_PK_SIZE; //RO
...
///      Access level: user level
////////////////////////////////////////////////////////////////////////////////////////////////////
#define UPG_FILE_INFO_SIZE      12
#define UPG_FILE_OWNER_MSPF1611  0
#define UPG_FILE_OWNER_MSPF449   1
uint8 UDO_startDownload(uint16 p_unBlockSize, uint32 p_ulDwldSize, uint8 p_ucOpMode)
{
  ...
  //UPG_FILE_INFO_SIZE + 6 -> to include in first downloaded block also the first Flash block length and address for validations
  //? where are those validations - avram
  if( p_unBlockSize > c_unMaxBlockSize || UPG_FILE_INFO_SIZE > p_unBlockSize )
  ...
  if( p_ucOpMode )
    return SFC_INVALID_PARAMETER; // see page 382 in ISA100May release
...
#if   (DEVICE_TYPE == DEV_TYPE_MC13225) 
    SelectCodePageAddress(); //get the dest for the new code 
    SSI_EraseSector_M25P40(g_ulCodeDestAddress);   
#endif
...
uint8 UDO_downloadData(uint16 p_unCrtBlockNumber, uint8 p_ucDataSize, uint8* p_pucData)
{
  ...
  {
    //the first packet contain also the informations related to the upgrade
    //tbd: need validation of these informations

    p_pucData += UPG_FILE_INFO_SIZE;

#if   (DEVICE_TYPE != DEV_TYPE_MC13225)
    //validate the Flash address of the first block
    //the firts two bytes represent the block length
    uint32 ulFlashStartAddr = ((uint32)p_pucData[2] << 24) |
                              ((uint32)p_pucData[3] << 16) |
                              ((uint32)p_pucData[4] << 8) |
                              p_pucData[5];

    if( FLASH_PROG_START_ADDR != ulFlashStartAddr )
        return SFC_BLOCK_DATA_ERROR;
#endif    

    // copy data to EEPROM starting from eeprom address EEP_FW_CODE_ADDR;
#if   (DEVICE_TYPE == DEV_TYPE_MC13225) 
    SSI_WriteBuffer_M25P40(p_pucData, g_ulCodeDestAddress + ulCrtEepAddr, p_ucDataSize - UPG_FILE_INFO_SIZE);
    SSI_EraseSector_M25P40(g_ulCodeDestAddress + 64*1024);   
    
#elif (DEVICE_TYPE == DEV_TYPE_MSP430F2618) 
    SPI_eeprom_write(ulCrtEepAddr, p_pucData, p_ucDataSize - UPG_FILE_INFO_SIZE);
    
#elif (DEVICE_TYPE == DEV_TYPE_AP91SAM7X512) 
    SPI_EraseEEPROMSector(0);
    SPI_EraseEEPROMSector(1); // 128 kb flash used for code
    
    SPI_WriteEEPROM(p_pucData, ulCrtEepAddr, p_ucDataSize - UPG_FILE_INFO_SIZE);
  ...
  {
    ulCrtEepAddr += (p_unCrtBlockNumber - 1) * g_unBlockSize - UPG_FILE_INFO_SIZE;
    // copy data to EEPROM starting from eeprom address EEP_FW_CODE_ADDR;

#if   (DEVICE_TYPE == DEV_TYPE_MC13225) 
    SSI_WriteBuffer_M25P40(p_pucData, g_ulCodeDestAddress + ulCrtEepAddr, p_ucDataSize);

#elif (DEVICE_TYPE == DEV_TYPE_MSP430F2618) 
    SPI_eeprom_write(ulCrtEepAddr, p_pucData, p_ucDataSize);
    
#elif (DEVICE_TYPE == DEV_TYPE_AP91SAM7X512) 
    SPI_WriteEEPROM(p_pucData, ulCrtEepAddr, p_ucDataSize);    
#else
  #warning  "UDO_downloadData not supported"   
#endif    
  }

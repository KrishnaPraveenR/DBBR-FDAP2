...
unsigned char Ajil111[250];
uint8 Ajil2222 = 0;
unsigned char Ajil444[150];
////////////////////////////////////////////////////////////////////////////////////////////////////
/// @author NIVIS LLC, Ion Ticus
/// @brief  It is used by the Application Layer to send an packet to one device
/// @param  p_pEUI64DestAddr     - final EUI64 destination
...
//                TLDE_DATA_Confirm( p_appHandle, SFC_NO_KEY );
  ...
  if( !pKey ) // not encryption
  {
      ...
      *(uint16*)stPseudoHdr.m_st.m_aPadding = __swap_bytes(ulUdpCkSum);      
      ...
      memcpy(&Ajil444[0], (const uint8*)&stPseudoHdr.m_st, sizeof(stPseudoHdr.m_st));     
      if( p_unAppDataLen > 1 )
      {      
          ulUdpCkSum = IcmpInterimChksum( p_pAppData +1 , p_unAppDataLen - 1 , ulUdpCkSum );
          memcpy(Ajil444 + sizeof(stPseudoHdr.m_st), p_pAppData + 1, p_unAppDataLen - 1);     
      }
  ...
  memset(&Ajil111, 0, sizeof(Ajil111));
  memcpy(&Ajil111,p_pRspPayload, p_unAppDataLen + unHdrLen);
                             ...
                             void *   p_pTLData )
{
    ...
    return AES_Decrypt_User( p_pKey->m_aKey,
                                    ...
                                    p_unTLDataLen -  p_unSecHeaderLen, GetMicSize(TMIC) );
}
...
void TLDE_encryptTLPayload( uint16 p_unAppDataLen, void * p_pAppData, const IPV6_PSEUDO_HDR * p_pAuth, uint16 p_unSecHeaderLen, const uint8 * p_pKey, uint32 p_ulCrtTai )
{
    ...
    //we must use the Script Server EUI64
    ...
    AES_Crypt_User( p_pKey,
                    ...
                    (const uint8 *) p_pAuth,
                     unAuthLen,
                     p_pAppData,
                     p_unAppDataLen, GetMicSize(TMIC));

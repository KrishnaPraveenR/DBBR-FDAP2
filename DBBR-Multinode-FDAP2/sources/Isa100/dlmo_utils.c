...
uint8 DLMO_ReadCandidates(uint16 p_unAttrID, uint16* p_punSize, uint8* p_pBuf)
{
  ...
  *(p_pBuf++) = stCandidates.m_ucCandidateNo;
  for(uint8 ucIdx=0; ucIdx<stCandidates.m_ucCandidateNo; ucIdx++)
  {
    ...
    *(p_pBuf++) = (uint8) stCandidates.m_cRSSI[ucIdx];     
    *(p_pBuf++) = stCandidates.m_ucRSQI[ucIdx]; 
    if (p_unAttrID == DL_EXTN_CANDIDATES)
    {
      //if (IsBackBone())
      {
        *(p_pBuf++) = (uint8) ((stCandidates.m_unClockDrift[ucIdx] >> 8) & 0xFF); 
        *(p_pBuf++) = (uint8) (stCandidates.m_unClockDrift[ucIdx] & 0xFF); 
      }
    }
  }  
...
uint8 DLMO_ReadDeviceCapability(uint16 p_unAttrID, uint16* p_punSize, uint8* p_pBuf)
{
  ...
  *(p_pBuf++) = stDevCap.m_ucSupportedCCAmodes;  //ISA2011 change  
  *(p_pBuf++) = stDevCap.m_ucOptions; // | (IsNtpSyncCapable()?((g_ulMaxRoundTripDelay/2) << 4):0xF0);
...
uint8 DLMO_SetSMIBChannelEntry(uint16* p_pSize, const uint8* p_pBuf, DLMO_SMIB_ENTRY* p_pEntry)
{
  ...
  if (pCh->m_unUID > 127)  //ISA 2011 limits dlmo.ch and dlmo.TsTemplate index to be less than or equal to 127
  {
    return SFC_INVALID_SIZE;    
  }    
...
{  
  ...
  if (pTs->m_unUID > 127)  //ISA 2011 limits dlmo.ch and dlmo.TsTemplate index to be less than or equal to 127
  {
    return SFC_INVALID_SIZE;    
  }      
...
uint8 DLMO_SetSMIBNeighborEntry(uint16* p_pSize, const uint8* p_pBuf, DLMO_SMIB_ENTRY* p_pEntry)
{
  ...
  pNei->m_ucGroupCode1 = *(p_pBuf++);
  pNei->m_ucGroupCode2 = *(p_pBuf++);  
  ...
  if(pNei->m_ucInfo & DLL_MASK_NEILINKBACKLOG)
  {
      ...
      pNei->m_ucLinkBacklogActivate = *(p_pBuf++);      
  }  
...
uint8 DLMO_GetSMIBNeighborEntry(const DLMO_SMIB_ENTRY* p_pEntry, uint8* p_pRspBuf)
{
  ...
  *(p_pRspBuf++) = pNei->m_ucGroupCode1;
  *(p_pRspBuf++) = pNei->m_ucGroupCode2;  
  *(p_pRspBuf++) = pNei->m_ucInfo;
  ...
  if(pNei->m_ucInfo & DLL_MASK_NEILINKBACKLOG)
  {
    ...
    *(p_pRspBuf++) = pNei->m_ucLinkBacklogActivate;              
  }  

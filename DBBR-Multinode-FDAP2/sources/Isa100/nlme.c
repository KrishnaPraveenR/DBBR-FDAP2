...
NLME_ADDR_TRANS_ATTRIBUTES * NLME_FindATT ( const uint8 * p_pIpv6Addr )
{
  ...
  uint8 NLME_IsInSameSubnet( uint16 p_unDestAddr )
  {
        ...
        NLME_ADDR_TRANS_ATTRIBUTES * pAtt = NLME_FindATTByShortAddr( (uint8*)&p_unDestAddr );
        
        if( pAtt )
        {
            NLME_ROUTE_ATTRIBUTES * pRoute = NLME_FindDestinationRoute( pAtt->m_aIPV6Addr );
            return  ( pRoute && !pRoute-> m_bOutgoingInterface ); // route found and is to DL
        }
        return 0;

...
#define SUPPORTED_CCA_MODE_1 0x01
#define SUPPORTED_CCA_MODE_2 0x02
#define SUPPORTED_CCA_MODE_3 0x04
...
#define DLL_RADIO_TRANSMIT_POWER_DEFAULT  36  // units dBm
#define DLL_RADIO_TRANSMIT_POWER_LOW      -20
#define DLL_RADIO_TRANSMIT_POWER_HIGH     36  

#define DLL_COUNTRY_CODE_DEFAULT        0x3C00
...
{  
  uint16  m_unUID;
  uint8   m_ucGroupCode1;  // may associate a group code with a set of neighbours
  uint8   m_ucGroupCode2;  // may associate a group code with a set of neighbours
  ...
  uint8 m_ucLinkBacklogActivate; //number of DPDU packets in the queue - ISA2011  
...
typedef struct
{
  ...
  int8    m_cRSSI[DLL_MAX_CANDIDATE_NO];  
  uint8   m_ucRSQI[DLL_MAX_CANDIDATE_NO];
  uint16  m_unClockDrift[DLL_MAX_CANDIDATE_NO];
} DLL_MIB_CANDIDATES;
...
#if (DEVICE_TYPE == DEV_TYPE_MSP430F2618)
  // #define DLME_QUEUE_PROTECTION_ENABLED
#endif

...
#ifdef UT_ACTIVED
    #if( UT_ACTIVED == UT_DLDE1_ONLY )
        #include "../UnitTests/DLDE/unit_test_DLDE1.h"
        DATA_CONFIRM  stConfStatus;
        DATA_INDICATE stDataIndicate;
    #endif
#endif // UT_ACTIVED
...
DLL_HASH_STRUCT *g_pCrtHashEntry;
  ...
  DLL_HASH_STRUCT * g_pCandidateHashEntry;
#endif  
uint8 g_ucTxNeighborIdx;

#ifdef DEBUG_ACK_ERROR
  uint8 g_aucDebug[72];
  uint8 g_ucDebugLastACKType;
#endif

DLL_DPDU * g_pDiscardMsg;

#ifdef PREDICTIVE_SCHEDULE_LINK
/* predicitve schedule link: the next active link */
NEXT_LNK g_stNextLink;
#endif
...
#ifdef DEBUG_ACK_ERROR
  memset(g_aucDebug, 0xFF, sizeof(g_aucDebug));
#endif  
}
...
void DLDE_remMsgFromQueue( DLL_DPDU * p_pDllMsg, uint8 p_ucConfirmReason )
{
  if ( (g_aDllMsgQueue <= p_pDllMsg) && (p_pDllMsg < g_pDllMsgQueueEnd) )
  {
      ...
      //update the WCI message
      g_stWciRcvRfMsg.m_u.m_stDllCfm.ucTxRetryCount = p_pDllMsg->m_stHdrs.m_ucRetryNo;
    #ifdef DEBUG_MSG_DELAY
      g_stWciRcvRfMsg.m_unDelayMilliSec = p_pDllMsg->m_stHdrs.unMilliSecDelay;       
    #endif
      //TBD - update also the DLL headers
    ...
    else if( ucTypeOptions & DLL_LNK_GROUP_MASK ) // DLL_GROUP
    {
        ...
        {           
            if( 2 == pMsg->m_stHdrs.m_ucPeerAddrLen )
            {
                ...
                if( pNeighbor && (( ucGroupCode == pNeighbor->m_ucGroupCode1 ) || ( ucGroupCode == pNeighbor->m_ucGroupCode2 )) )                  
                {
                    if( p_pLink->m_mfLinkType & DLL_MASK_LNKIDLE )
                    {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
/// @author LLC Nivis, Dorin Pavel, Ion Ticus
/// @brief  find the first BIT=1 within a bitmap
/// @param  p_ulBitmap - passed bitmap           
/// @return index of BIT=1 within the bitmap
/// @remarks
///      Access level: Interrupt
////////////////////////////////////////////////////////////////////////////////////////////////////
static uint8 DLDE_ComputeNextLinkIfBitmap (uint32 p_ulBitmap)
{  
  uint8 ucIdx = 0;
  
  for (; p_ulBitmap; ucIdx++, p_ulBitmap >>= 1)
  {
    if ( p_ulBitmap & 1L )
      return ucIdx;
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
/// @author LLC Nivis, Dorin Pavel
/// @brief  predictive link scheduler, see when is the next active link
/// @param  none
/// @return none
/// @remarks
///      Access level: Interrupt
////////////////////////////////////////////////////////////////////////////////////////////////////
static void DLDE_PredictiveScheduleLink (void)
{ 
  uint16 unDiff;
  DLL_HASH_STRUCT *pCrtHashStruct=g_stDll.m_aHashTable;
  
  // ignore scheduled TX links if DLL msg queue is empty and not active Clk integration
  uint8 ucIgnoreTxLinks = (g_aDllMsgQueue == g_pDllMsgQueueEnd) && (g_stTAI.m_ucClkInterogationStatus == MLSM_CLK_INTEROGATION_OFF);
    
  g_stNextLink.m_unSlotsUntilWakeUp = 0xFFFF;
  
  for( ;pCrtHashStruct < g_stDll.m_aHashTable+g_stDll.m_ucHashTableSize; pCrtHashStruct ++ )
  {    
    if( ucIgnoreTxLinks )
    {
        // ignore if not RX link and not TX advertise
        if( !(pCrtHashStruct->m_ucLinkType & DLL_MASK_LNKRX)  
            && !((pCrtHashStruct->m_ucLinkType & DLL_MASK_LNKDISC) && ((pCrtHashStruct->m_ucLinkType & DLL_MASK_LNKDISC) != DLL_MASK_LNKDISC)) )
        {
            continue;               
        }
    }
    
    uint16 unSlotNo = g_aDllSuperframesTable[pCrtHashStruct->m_ucSuperframeIdx].m_stSuperframe.m_unCrtOffset + 1;
    uint16 unSfSize = g_aDllSuperframesTable[pCrtHashStruct->m_ucSuperframeIdx].m_stSuperframe.m_unSfPeriod;
    if( unSlotNo >= unSfSize )
    {
      unSlotNo = 0;
    }
    ...
    if( ucLinkType != (3 << DLL_ROT_LNKSC) ) // not bitmap link
    {
      if( unSlotNo <= pLink->m_aSchedule.m_aOffset.m_unOffset ) // before start offset
      {
        unDiff = pLink->m_aSchedule.m_aOffset.m_unOffset - unSlotNo;
      }
      else // after start
      {            
        if ( (1 << DLL_ROT_LNKSC) == ucLinkType ) // 1 - offset & interval
        {
          uint16 unSlotDiff = (unSlotNo - pLink->m_aSchedule.m_aOffsetInterval.m_unOffset) %  pLink->m_aSchedule.m_aOffsetInterval.m_unInterval;
          
          if( !unSlotDiff )
          {
              unDiff = 0;
          }
          else if( unSlotNo - unSlotDiff + pLink->m_aSchedule.m_aOffsetInterval.m_unInterval < unSfSize )
          { // next start time
              unDiff = pLink->m_aSchedule.m_aOffsetInterval.m_unInterval - unSlotDiff;     
          }
          else
          { // next start time
              unDiff = unSfSize - unSlotNo + pLink->m_aSchedule.m_aOffsetInterval.m_unOffset; 
          } 
        }
        else // 0 - offset only, 2 - range 
        { // next start time
          ...
          {
              unDiff = unSfSize - unSlotNo + pLink->m_aSchedule.m_aOffsetInterval.m_unOffset;  // 2 - range: m_unFirst = m_unOffset
          }
        }
      }  
    }
    else // 3 - bitmap
    {
      if( unSlotNo < 32 )
      {
          unDiff = DLDE_ComputeNextLinkIfBitmap( pLink->m_aSchedule.m_ulBitmap >> unSlotNo );
      }
      else
      {
           unDiff = 32;
      ...
      if( unDiff == 32 )
      {  
        unDiff = unSfSize - unSlotNo + DLDE_ComputeNextLinkIfBitmap(pLink->m_aSchedule.m_ulBitmap);
      }
    }

    unDiff ++;
    if( unDiff < g_stNextLink.m_unSlotsUntilWakeUp )
    {
        g_stNextLink.m_unSlotsUntilWakeUp  = unDiff;
    }
  }   
}   
...
{  
  ...
  for( ;pCrtHashStruct < g_stDll.m_aHashTable+g_stDll.m_ucHashTableSize; pCrtHashStruct ++ )
  {
    ...
    {   
        ...
        {                    
            ...
            if( ucLinkType & DLL_MASK_LNKEXP )
            {
                ...
                {
                    //TODO - if link uses graph or neighbor group need to check if another message available to be sent to another neighbor  
        ...
        else if( DEVICE_JOINED == g_ucDiscoveryState )// not message found on a joined device
        {
            ...
            if( (g_stTAI.m_ucClkInterogationStatus == MLSM_CLK_INTEROGATION_ON) 
                  && (g_ucTxNeighborIdx != 0xFF) 
                  && (g_aDllNeighborsTable[g_ucTxNeighborIdx].m_stNeighbor.m_ucInfo & DLL_MASK_NEICLKSRC) )
            {
                // no message but send a blank message to the preffered/secondary source clock for ACK time synchronization
                return SFC_TX_LINK;
            }
#endif            
    ...
    else if (ucLinkType & DLL_MASK_LNKDISC) // a discovery link
    { 
        return SFC_TX_LINK;
    }
    ...
    else // tbd (delegated link)
    { 
    }
  }

#ifdef PREDICTIVE_SCHEDULE_LINK  
  // if no active link found -> see when is the next active link 
  DLDE_PredictiveScheduleLink();
#endif
                       ...
                       HANDLE       p_hHandle)
{
  ...
  // check if the short srcAddr is available
//  if (DLL_NEXT_HOP_HANDLE == p_hHandle && p_unContractID != JOIN_CONTRACT_ID)
//  {
//     DLDE_Data_Confirm(p_hHandle, SFC_INVALID_PARAMETER);
//     return;
//  }
#ifdef DEBUG_MSG_DELAY   
  stMsgHrds.unMilliSecDelay = g_stWciRcvRfMsg.m_unDelayMilliSec = GetMSecMessageDelay(g_ulMsgTaiSec, g_ulMsgTaiMicroSec);
#endif
  
  if (p_ucPayloadLength > g_ucMaxDSDUSize )
  {
     g_stWciRcvRfMsg.m_u.m_stDllCfm.m_stDL.m_ucLen = 0;
     g_stWciRcvRfMsg.m_ulRxSlotTAISec = 0;
     g_stWciRcvRfMsg.m_unRxSlotTAIFract = 0;
...
unsigned int DLDE_CheckForwardQueueLimitation( uint8 p_ucPriority, uint8* p_pucReason )
{
  ...
  *p_pucReason = 0;  
  ...
  {              
      ...
      // find the dlmo11a.QueuePriority magic number  
...
__monitor uint8 DLDE_addMsgWithLinkCk (const DLL_MSG_HRDS* p_pMsgHrds, const uint8 * p_pPayload )
{
    ...
    {
        if( !g_ucDllQueueOrdering )
        {
            //FIFO
            memcpy( &g_pDllMsgQueueEnd->m_stHdrs, p_pMsgHrds, sizeof(g_pDllMsgQueueEnd->m_stHdrs) );      
            memcpy( g_pDllMsgQueueEnd->m_aPayload, p_pPayload, p_pMsgHrds->m_ucPayloadLen );
            
            g_pDllMsgQueueEnd->m_stHdrs.m_ucRetryNo = 0;
        }
        else
        {
            DLL_DPDU * pMsg = g_aDllMsgQueue;
...
void DLDE_HandleRxMessage( uint8 * p_pRxBuff )
{
    ...
    if ((g_ucDlSecurityControl & 0x07) & SECURITY_ENC) 
    {
    #ifdef BBR1_HW
        CC2420_ReadDecryptedBuffer( pPayload, g_stDllMsgHrds.m_ucPayloadLen );
    #endif
    #ifdef BBR2_HW
    	CC2520_ReadDecryptedBuffer( pPayload, g_stDllMsgHrds.m_ucPayloadLen );
    #endif
    }
#endif
...
//                           ucPriorityAndFlags,                // priority and flags
   ...
   { 
        ...
        {                    
            ...
            DbgError( SFC_OPERATION_ABORTED );
        ...
        *(pDROUT++) -= 1;
        ...
        // g_stDllMsgHrds.m_ucPriority = (g_stDllMsgHrds.m_aDROUT[0] & 0x78) >> 3; // this is done in MLDE_IncomingFrame()
        ...
        else
        {
            ...
            {
                DbgError( SFC_INVALID_BLOCK_SIZE );
                return;
            }
...
//    return 0xFF; // something wrong with the packet route but why don't try direct ? (because may don't have TX link?)
    return ucNeighbor; 
}
                          ...
                          const uint8 * p_pRxBuff )
{
  ...
  {  
      do
      {
          ...
          //update the WCI RX_DLL_ACK
          g_stWciRcvAckMsg.m_u.m_stDllAck.m_stDL.m_ucLen = unFrameLength - MIC_SIZE;  //without MMIC
          memcpy(g_stWciRcvAckMsg.m_u.m_stDllAck.m_stDL.m_aBuff, p_pRxBuff, g_stWciRcvAckMsg.m_u.m_stDllAck.m_stDL.m_ucLen);
          g_stWciRcvAckMsg.m_unRxSFDFract = g_unRxAckSFDOffsetFract;
          memset( g_stWciRcvAckMsg.m_MMIC, 0, MAX_DMIC_SIZE );  
          memcpy( g_stWciRcvAckMsg.m_MMIC, p_pRxBuff + g_stWciRcvAckMsg.m_u.m_stDllAck.m_stDL.m_ucLen, MIC_SIZE );  
          g_stWciRcvAckMsg.m_ucChannel = g_ucDllCrtCh;
          
#ifdef CC2420_AES_H
          #define aucAuthData stAligned.m_aucAuthData
#else
         uint8 aucAuthData[MAX_ACK_LEN + 4];   //including virtual MIC field 
...
#ifdef CC2420_AES_H
            uint8 m_aucAuthData[MAX_ACK_LEN + 4];   //including virtual MIC field
#endif            
          ...
          if( (ucDHRFrameCtrl & (BIT2 | BIT1 | BIT0) ) != DLL_MASK_DL_VERSION_ACK ) //ISA 2011 Bit2 is reserved = 0 :: Bit1,Bit0 says ACK/NACK
          {
...
//          PHY_ReleaseRXBuff();
    #ifdef BBR1_HW
          CC2420_LoadRXBuffer( aucAuthData, unFrameLength + 4);
    #endif          
    #ifdef BBR2_HW
    	  CC2520_LoadRXBuffer( aucAuthData, unFrameLength + 4);
    #endif
#endif
          DLL_SMIB_NEIGHBOR * pNeighbor;
          ...
          *(uint32*)(stAligned.m_aucNonce + 8) = MLSM_GetSlotStartMsBE(nSlotDiff);
          ...
          stAligned.m_aucNonce[12] = (p_pRxBuff[2] & 0x07);    //Nonce[12] --> bit7 (0) + channel(bit6 to 3) + seq no (bit2 to 0)                  
          stAligned.m_aucNonce[12] |= (g_ucDllCrtCh << 3);                   
    
#ifdef CC2420_AES_H           
          PHY_AES_SetDecryptNonce( stAligned.m_aucNonce );
          PHY_AES_SetDecryptPlainLen(unFrameLength);
          #ifdef BBR1_HW
          CC2420_StartDecryptRXBuffer(CC2420_SECCTRL0_SEC_M_4_BYTES);
          
           if(AES_SUCCESS != CC2420_CkDecryptRXBuffer( unFrameLength + 4 ) )
           {
              SLME_DL_FailReport();
          ...
          #endif
          #ifdef BBR2_HW
          CC2520_StartDecryptAckBuffer();
           if(AES_SUCCESS != CC2520_CkDecryptRXBuffer( unFrameLength + 4 ) )
           {
              SLME_DL_FailReport();
              break;
           }
        ...
        if(TxRFMsgFlag == 1)
        { 
          //888 WCI_AddMessage( sizeof(g_stWciTxRfMsg), (const uint8*)&g_stWciTxRfMsg );
          TxRFMsgFlag = 0;
        } 
         if( DLL_LOGGING_LEVEL <= g_ucLoggingLevel )
         {
             //add the received DLL ACK
             g_stWciRcvAckMsg.m_ucMsgType = WCITXT_MSG_TO_BBR_AS_RX_ACK;
...
//else if (p_unAckTypeErrCode == ((DUOCAST_DEST << 8) | SUCCESS) )
//{
//    not supported by ISA100 (yet)
//}
  else // not success or BCAST 
  {
      #warning " CHange the if condition for broadcast message"
      //check the shared link
  ...
  #ifndef WCI_DLL_COMPLIANCE
      if (SFC_SUCCESS != ucSFC)
      {
          DLDE_remMsgFromQueue( g_pDllMsg, ucSFC );
      }
  #endif // #ifndef WCI_DLL_COMPLIANCE
}
...
uint8 DLDE_retryMsg( void )
{
  ...
  {
#ifdef DEBUG_ACK_ERROR
      memmove(g_aucDebug+12, g_aucDebug, sizeof(g_aucDebug) - 12);
      
      g_aucDebug[0] = g_pDllMsg->m_stHdrs.m_hHandle >> 8;
      g_aucDebug[1] = g_pDllMsg->m_stHdrs.m_hHandle;
      g_aucDebug[2] = g_aDllLinksTable[g_pCrtHashEntry->m_ucLinkIdx].m_stLink.m_unUID >> 8;
      g_aucDebug[3] = g_aDllLinksTable[g_pCrtHashEntry->m_ucLinkIdx].m_stLink.m_unUID;
      g_aucDebug[4] = g_pCrtHashEntry->m_ucLinkType;
      g_aucDebug[5] = g_pDllMsg->m_stHdrs.m_unGraphID;                                                  // only low byte; will be 0xFF if no graph
      g_aucDebug[6] = g_aDllNeighborsTable[g_pDllMsg->m_stHdrs.m_ucNeighborIdx].m_stNeighbor.m_unUID;   // only low byte;
      g_aucDebug[7] = g_ucDebugLastACKType;
      g_aucDebug[8] = g_ucDllCrtCh;
      g_aucDebug[9] = g_aDllSuperframesTable[g_pCrtHashEntry->m_ucSuperframeIdx].m_stSuperframe.m_unCrtOffset >> 8;  
      g_aucDebug[10] = g_aDllSuperframesTable[g_pCrtHashEntry->m_ucSuperframeIdx].m_stSuperframe.m_unCrtOffset;
      g_aucDebug[11] = g_aDllSuperframesTable[g_pCrtHashEntry->m_ucSuperframeIdx].m_stSuperframe.m_ucChSeqLen;
#endif      
      ...
      if( g_pDllMsg->m_stHdrs.m_unGraphID != 0xFFFF )
      {
          ...
          {  
          ...
          #else    
              if( g_pDllMsg->m_stHdrs.m_ucRetryNo < (MAX_TX_NO_TO_ONE_NEIHBOR * pGraph->m_stGraph.m_ucNeighCount) )
              {              
                  DLDE_setMsgGraphNeighbor(&g_pDllMsg->m_stHdrs, pGraph );
                  if( g_pDllMsg->m_stHdrs.m_ucNeighborIdx < 0xFF )
                  {
              ...
              {                
                  if (pGraph->m_stGraph.m_ucNeighCount)
                  {
                      return SFC_RETRY_NO_EXPIRED_ON_GRAPH;
                  }
                  else
                  {
                      return SFC_FAILURE; 
      ...
      else if( g_pDllMsg->m_stHdrs.m_ucRetryNo >= MAX_TX_NO_TO_ONE_NEIHBOR )  // the message is not GRAPH-routed. let the DLL try again on the same link
      {
          // nothing left when decrement -> stop retry             
          return SFC_RETRY_NO_EXPIRED_ON_SRC;      
      }
  }
...
{
  #ifdef CYCLIC_GRAPH_RETRY 
    uint8  ucIdxInGraph = p_ucRetryNo % p_pstGraph->m_stGraph.m_ucNeighCount;   
  #else            // not CYCLIC_GRAPH_RETRY
    uint8  ucIdxInGraph = p_ucRetryNo / MAX_TX_NO_TO_ONE_NEIHBOR;              
  #endif // CYCLIC_GRAPH_RETRY

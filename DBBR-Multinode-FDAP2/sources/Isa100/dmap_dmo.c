...
const uint8 c_ucCommSWMinorVer  = 1;          // assigned by ISA100

#define STRING_VERSION '0','4','.','0','1','.','1','1'

#ifdef BACKBONE_SUPPORT
    const uint8 c_aucSWRevInfo[12]    = {'B','B','_','_', STRING_VERSION };
#elif ( PLATFORM == PLATFORM_GE_DEVICE )
    const uint8 c_aucSWRevInfo[12]    = {'G','E','_','_', STRING_VERSION };
#elif ( PLATFORM == PLATFORM_YOKOGAWA )
    const uint8 c_aucSWRevInfo[12]    = {'Y','K','_','_', STRING_VERSION };
#elif ( PLATFORM == PLATFORM_DEVELOPMENT )
    const uint8 c_aucSWRevInfo[12]    = {'D','V','_','_', STRING_VERSION };
#else
    const uint8 c_aucSWRevInfo[12]    = {'I','K','_','_', STRING_VERSION };
#endif
...
const uint8 c_aucObjList[DMAP_OBJ_NO*5] = 
{
  ...
  (uint16)(DMAP_CUST_PROV_OBJ_ID) >> 8,  (uint8)(DMAP_CUST_PROV_OBJ_ID),  (uint8)(OBJ_TYPE_CUST_PROV),  0, 0,
};
...
{   
  ...
  DMO_CONTRACT_ATTRIBUTE * pContract = DMO_GetContract( p_pDstAddr128,
                                                        p_unDstTLPort,
                                                        p_unSrcTLPort,                        
                                                        p_ucSrvcType );
  if (pContract)
  ...
  // in order to save memory, save theese values in the next free contract table entry
  pContract = g_stDMO.m_aContractTbl+g_stDMO.m_ucContractNo;  
  ...
  if( g_stDSMO.m_ucTLSecurityLevel != SECURITY_NONE )
  {
      ...
      {            
          if (SFC_SUCCESS == SLME_GenerateNewSessionKeyRequest(p_pDstAddr128,ucUdpPorts))
          {
              ...
              g_unTimeout = (uint16)(MLSM_GetCrtTaiSec());
...
{  
  ...
  *(pReqBuf++) = p_pContract->m_ucServiceType; 
  ...
  // destination 128 bit addres and destination port
...
{  
  ...
  // inform other UAPs about this new contract  
#if ( _UAP_TYPE == UAP_TYPE_ISA100_API )
  if( g_ucUapId && (pContract->m_unSrcTLPort == (ISA100_DMAP_PORT + g_ucUapId)) ) // it is application processor contract 
  {
      if( SFC_SUCCESS != p_pExecRsp->m_ucSFC )
      {
          uint8 aucTemp[2] = { g_ucUAPContractReqID, p_pExecRsp->m_ucSFC };           
          UAP_NotifyContractResponse( aucTemp, sizeof(aucTemp) );

      }
      else if( p_pExecRsp->p_pRspData[0] == g_ucContractReqID )
      {
...
#ifdef WCI_SUPPORT       
  //update the short address
  g_stDMO.m_unShortAddr = (uint16)c_oEUI64BE[6] << 8 | c_oEUI64BE[7];
  g_stDMO.m_unShortAddr &= 0x7FFF;    //just 15 bits for the short address
#endif  
#endif  
...
{    
    if( g_stDMO.m_ucJoinCommand != DMO_JOIN_CMD_NONE && JOIN_CMD_APPLY_ASAP_REQ_ID == g_unJoinCommandReqId ) // a join command is on progress (emulated exec command via write ) 
    {

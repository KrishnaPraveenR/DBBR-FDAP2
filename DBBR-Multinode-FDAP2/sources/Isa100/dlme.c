...
#elif(DEVICE_TYPE == DEV_TYPE_MSP430F2618 || DEVICE_TYPE == DEV_TYPE_AP91SAM7X512)
    //CC2420 modem

    #define MODEM_WORMUP_TMR2   USEC_TO_TMR0(144)
    #define MODEM_CCA_TMR2      USEC_TO_TMR0(144+192)
    #define MODEM_PREAMBLE_TMR2 USEC_TO_TMR0(5*32+32) // 5 = preamble + sfd + pipe delay

//    #define MODEM_WORMUP_TMR2   USEC_TO_TMR0(192)
//    #define MODEM_CCA_TMR2      USEC_TO_TMR0(92+128)
//    #define MODEM_PREAMBLE_TMR2 USEC_TO_TMR0(4*32)
...
extern uint8 MLSM_IsClockSourceAvailable( void );
uint8 g_ucAdvTimeSync = 0;
...
{      
    ...
    case SMIB_IDX_DL_NEIGH:   
        ...
        if (MLSM_IsClockSourceAvailable() == SFC_SUCCESS)
        {
          g_ucAdvTimeSync = 1;
        }
        else      
                       ...
                       uint8*   p_pBuf)
{
  ...
  if (DL_NEIGH_DIAG_META == p_unAttrID) // this is an exception, only for neighbor diagnostics
  {
      ...
      unDiagNo *= sizeof(DLL_SMIB_NEIGHBOR_DIAG);
      ...
      *(p_pBuf++) = unDiagNo;       // low byte
      ...
      unDiagNo = g_aDllTableConst[ucLocalID].m_ucSMIBMaxRecNo * sizeof(DLL_SMIB_NEIGHBOR_DIAG);
      *(p_pBuf++) = unDiagNo >> 8;  // high byte
      *(p_pBuf++) = unDiagNo;       // low byte 
  }
  else
...
__monitor uint8 DLME_SetMIBRequest(uint8 p_ucAttrID, const void* p_pValue)
{
  if( p_ucAttrID < DL_NO ) 
  {
      ...
      if ( ucLocalId < DL_NON_INDEXED_ATTR_NO )
      {
          ...
          if( (DL_LINK_PRIORITY_XMIT == p_ucAttrID) || (DL_LINK_PRIORITY_RCV == p_ucAttrID) )
          {
...
void DLME_addHashEntry( const DLL_HASH_STRUCT * p_pHashEntry )
{
    ...
    for( ;pCrtHashStruct < g_stDll.m_aHashTable+g_stDll.m_ucHashTableSize; pCrtHashStruct++ )
    {
        ...
        {
            memmove( pCrtHashStruct+1, pCrtHashStruct, (uint8*) ( g_stDll.m_aHashTable + g_stDll.m_ucHashTableSize ) - (uint8*) pCrtHashStruct );
            break;
        }
    }
    ...
    if( (p_ucAttrID == DL_SF_IDLE) || (p_ucAttrID == DL_NEIGH_DIAG_RESET) )
    {
        ...
        {        
            ...
            if( pSmib ) // founded
            {
                ...
                {
                    ((DLL_SMIB_SUPERFRAME*)pSmib)->m_lIdleTimer = ((DLL_SMIB_SF_IDLE*)(p_pValue+2))->m_lIdleTimer;
...
{
  #warning "Alert triggering mechanism is ambiguously described in ISA100 draft2A; trigger an alert if there is at least one candidate"
...
///      Access level: user level
////////////////////////////////////////////////////////////////////////////////////////////////////
void DLME_CheckChannelDiagAlarm(void)
{  
  ...
  // check if dl_connectivity alert is enabled and if a chDiag alarma has been sent previously 
...
///      Access level: user level
////////////////////////////////////////////////////////////////////////////////////////////////////
void DLME_CheckNeighborDiagAlarm(void)
{    

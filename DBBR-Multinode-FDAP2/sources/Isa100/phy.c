...
//uint8 g_ucLastLQI;
  ...
  extern uint8 g_aucDebugResetReason[];
#endif
...
uint16 g_unTxAckCountSuccess, g_unTxNakCountSuccess, g_unTxCountSuccess, g_unTxPktFailure;         
...
void PHY_OnTimeTriggeredAction(void)
{
  ...
  // Initialize the flag to zero during start of each reception and time stamp as invalid
  switch(g_ucXVRStatus)
  {
    ...
    case PHY_TX_START_DO:
        #if (DEBUG_MODE >= 3)
          DEBUG0_ON();    // test - start modem action
          DEBUG0_OFF();
          DEBUG0_ON();    // test - start modem action
          DEBUG0_OFF();
          DEBUG0_ON();    // test - start modem action
        ...
        #ifdef BBR2_HW
          CC2520_SetModemState(CC2520_MODEM_STATE_TX_NO_CCA);
        #endif        
    ...
    case PHY_TX_WITH_CCA_START_DO:
...
#ifdef BBR2_HW
      uint8 ucFSMstatus, ucstatusRegister;
      CC2520_SetModemState(CC2520_MODEM_STATE_RX);
      ...
      while (!(ucstatusRegister & CC2520_RSSI_VALID));
    
      CC2520_1_MACRO_SELECT();
      CC2520_1_MACRO_GETMEM(CC2520_FSMSTAT1,ucFSMstatus); 
      CC2520_1_MACRO_RELEASE();
      
      if ((ucFSMstatus & 0x10) == 0)
      {
        g_unTxPktFailure++;
  ...
  {    
    ...
    if( p_ucDisableLevel < g_ucXVRStatus ) // minimize the consuption sense 
    {
      ...
      if( p_ucDisableLevel == PHY_IDLE )
      {
        ...
        #endif
        #ifdef BBR2_HW
        // If transciever was in receive mode earlier check which radio to put in to idle mode
        if (PHY_RX_IN_PROGRESS == g_ucXVRStatus)
        {
        ...
        CC2520_SetModemState(CC2520_MODEM_STATE_IDLE);
        #endif        
      }
      else
      {
        ...
        #endif
        #ifdef BBR2_HW
           CC2520_SetModemState(CC2520_MODEM_STATE_POWERDOWN);
        #endif        
      }
...
void PHY_Reset(void)
{
    ...
    CC2420_MACRO_CLEAR_ALL_IRQS();
       ...
       _NOP();_NOP();
       _NOP();_NOP();
       CC2520_SetModemState(CC2520_MODEM_STATE_POWERUP);
...
void PHY_RX_Request ( unsigned char p_ucChannel, unsigned char p_ucRelativeFlag,  unsigned int p_unDelay )
{
    ...
    if( g_ucLastRFChannel != p_ucChannel )
    {
          ...
          {
            CC2520_1_MACRO_SELECT();
            CC2520_1_MACRO_SET_RFCH_0TO15(p_ucChannel);
            CC2520_1_MACRO_RELEASE();
          }
       #endif
...
void PHY_MNG_Request ( unsigned char p_ucService, unsigned char * p_pucData)
{
  ...
  switch ( p_ucService )
  {
    ...
    case PHY_MNG_WRITE_TX_PWR_LVL :
      ...
      #endif
      #ifdef BBR2_HW
        // New PA power level should be in range [0x00..0xFF] unsigned char
        if (g_ucRadioOperationSts & CC2520_RADIO_1)
        {
          CC2520_1_MACRO_SELECT();
          CC2520_1_MACRO_SET_PA( ((unsigned char)p_pucData[0]) );
          CC2520_1_MACRO_RELEASE();
        }
      #endif
      break;
...
void RF_Interrupt(void)
{
  ...
  if( g_ucXVRStatus == PHY_RX_IN_PROGRESS)
  {
      ...
      #endif
      #ifdef BBR2_HW
      if (CC2520_1_FIFO_IRQ_STATUS( ulPIOIsr )) // Check if FIFO Interrupt has come from either Radio 1 or Radio 2
      #endif
      {
      ...
      #endif
      #ifdef BBR2_HW
      else if (CC2520_1_SFD_IRQ_STATUS(ulPIOIsr) && CC2520_1_SFD_STATUS())
      #endif
      {
          return;
      }
      ...
      #endif
      #ifdef BBR2_HW
      if ((CC2520_1_FIFOP_IRQ_STATUS(ulPIOIsr) && CC2520_1_FIFOP_STATUS())) // Process FIFOP Interrupts with transition from 0 to 1 only
      #endif
       {
      #if (DEBUG_MODE >= 2)
          DEBUG3_OFF();  DEBUG3_ON(); 
...
//          CC2420_FIFOP_CLEAR_IRQ(); // Clear IRQ Flag
      ...
      #endif // end of BBR1_HW
          
      #ifdef BBR2_HW         
           // Get the data from Radio 1 Buffer only if it is available
           if (CC2520_1_FIFOP_STATUS() && (g_ucRadioOperationSts & CC2520_RADIO_1))
           {
             CC2520_1_FIFOP_IRQOFF(); 
             
             // read also the length
             CC2520_1_MACRO_SELECT();
             CC2520_1_MACRO_GETREG( CC2520_RSSI, g_ucLastRSSI ); // read RSSI from Radio 1
             CC2520_1_MACRO_RESELECT();
             CC2520_1_MACRO_READ_RAM_LITTLE_E((unsigned char*)g_aucDllRxTxBuf, 
                                              (unsigned int)CC2520_RAM_RXBUF, 
                                              1+PHY_MIN_HDR_SIZE+2 ); // + RSSI and CRC
             CC2520_1_MACRO_RELEASE();
             
             uint8 ucRxLen = g_aucDllRxTxBuf[0] - 2; // remove the RSSI and CRC from len
            
             // Rares: 802.15.4 frame len contains Payload max 125 bytes + 2 bytes FCS = max 127 bytes
             // Check the length of the incomming frame 6 <= len <= 127 on both the Radios
             if( (g_ucRadioOperationSts & CC2520_RADIO_1) && ((unsigned char)(ucRxLen-6) > (MAX_PHY_PAYLOAD_LEN-6))  )
             {
                 PHY_ErrorIndicate(RX_CRC_ERROR, NULL);
                 return;
             }
       ...
       #endif // end of BBR2
      } // end of FIFOP Interrupt check
      // SFD Pin Interrupt = packet end
      #ifdef BBR1_HW
      if (CC2420_SFD_IRQ_STATUS(ulPIOIsr))
      #endif
      #ifdef BBR2_HW
      if (CC2520_1_SFD_IRQ_STATUS(ulPIOIsr))
      #endif
      {
          #if (DEBUG_MODE >= 2)
          ...
          PHY_Disable_Request(PHY_IDLE); // stop reception 
            ...
            {
              CC2520_1_MACRO_SELECT();
              CC2520_1_MACRO_READ_RAM_LITTLE_E((unsigned char*)g_aucDllRxTxBuf+1+PHY_MIN_HDR_SIZE+2, 
                                               (unsigned int)CC2520_RAM_RXBUF+1+PHY_MIN_HDR_SIZE+2, 
                                               (g_aucDllRxTxBuf[0]-PHY_MIN_HDR_SIZE));
              CC2520_1_MACRO_RELEASE();
            }
  ...
  {      
    ...
    if (CC2520_RADIO_1 == eActiveTransmitter)
      {
        // Read the exception register for TX UNDERFLOW FLAG
        CC2520_1_MACRO_GETREG(CC2520_EXCFLAG0, temp);
        CC2520_1_MACRO_RELEASE();
        
        if ( temp & CC2520_EXCEP_TX_UNDERFLOW )
        {
          // ERROR ! TX Underflow

          // Flush the TXFIFO buffer. This is also clearing the TX Underflow flag
          CC2520_1_MACRO_SELECT();
            CC2520_1_MACRO_FLUSH_TXFIFO();
            CC2520_1_MACRO_STROBE(CC2520_SRFOFF);        // Turn Off the RF (CC2520 goes to Idle state)
          CC2520_1_MACRO_RELEASE();
    ...
    #endif
    #ifdef BBR2_HW
      if (CC2520_1_SFD_IRQ_STATUS(ulPIOIsr) && !CC2520_1_SFD_STATUS()) 
    #endif
      {
          PHY_Disable_Request(PHY_IDLE);
      #ifdef BBR1_HW
          ...
          if( g_ucDllTdmaStat == MLSM_TX_ACK )
          {
              ...
              g_unTxAckCountSuccess++;
          }
          else if( g_ucDllTdmaStat == MLSM_TX_NACK )
          {
              g_unTxNakCountSuccess++;
...
{  
  if( g_pPrevKey != p_ucKey )
  {
    ...
    if (CC2520_RADIO_1 == eActiveTransmitter)
    {
      ...
      // Select KEY0 as current decryption key
      CC2520_1_MACRO_SET_ENCKEY0(p_ucKey);   // Load the key in RAM by overwriting KEY0 location      
...
void PHY_AES_SetDecryptNonce( const unsigned char* p_ucNonce )
{
  ...
  CC2420_MACRO_RELEASE();
   ...
   if (CC2520_RADIO_1 == eActiveTransmitter)
    {
      CC2520_1_MACRO_SELECT();
        CC2520_1_MACRO_SET_RX_NONCE(0x09,p_ucNonce,0x00,0x01);            // Load the Nonce in RAM to RXNONCE  
      CC2520_1_MACRO_RELEASE();
    }
  #endif
}
...
void PHY_AES_SetDecryptPlainLen( unsigned char p_ucPlainRx )
{
    ...
    if (CC2520_RADIO_1 == eActiveTransmitter)
    {
      CC2520_1_MACRO_SET_AES_RX_PLAINTEXT(p_ucPlainRx); // Set the # of plaintext bytes between frame len and 1rst byte to decrypt      
    }
  #endif
}
...
void PHY_AES_SetEncryptNonce( const unsigned char* p_ucNonce, unsigned char p_ucPlainTx )
{
    ...
    {
      CC2520_1_MACRO_SELECT();
      CC2520_1_MACRO_SET_AES_TX_PLAINTEXT(p_ucPlainTx); // Set the # of plaintext bytes between frame len and 1rst byte to decrypt      
      CC2520_1_MACRO_SET_TX_NONCE(0x09,p_ucNonce,0x00,0x01);            // Load the Nonce in RAM to TXNONCE  
      CC2520_1_MACRO_RELEASE();
    }
   #endif
}
...
{
  #ifdef BBR1_HW                       
  volatile uint16 unTmp = 0;   
  
  if ( g_ucDiscoveryState == DEVICE_JOINED )
  {
    CC2420_MACRO_SELECT();  
    CC2420_MACRO_GETREG( CC2420_SECCTRL0, unTmp );
    CC2420_MACRO_RELEASE();
    
    // reinitialize PHY
    //if( unTmp !=  g_unCC2420SecCtrl_0)
    //{
    //  g_ucResetPhyChip = 1;
    //}
  }
  #endif
  #ifdef BBR2_HW                       
  volatile uint8 unTmp = 0;   
  
  if ( g_ucDiscoveryState == DEVICE_JOINED )
  {
    CC2520_1_MACRO_SELECT();
    CC2520_1_MACRO_GETMEM( CC2520_MDMCTRL0, unTmp)
    CC2520_1_MACRO_RELEASE();
                                 ...
                                 )
{
  ...
  PHY_SetToIdle(); // wake up if modem is down
    ...
    *(uint16*)(st.m_aA0+14) = __swap_bytes( p_unToDecryptLen );
    ...
    #ifdef BBR2_HW
    // encrypt the block using the expandedKey    
    CC2520_ComputeAES(  st.m_aA0,  st.m_aInput );     
    #endif
    ...
    *(uint16*)(st.m_aInput) ^= __swap_bytes( p_unToAuthOnlyLen );
    ...
    #endif
    #ifdef BBR2_HW
    CC2520_ComputeAES( st.m_aInput, st.m_aInput );
    #endif    
    ...
    *(uint16*)(st.m_aA0+14) = 0x0100; // 0x00 0x01 (network order)
    ...
    while(  p_unToDecryptLen )
    {
      ...
      #ifdef BBR2_HW
      // encrypt the block using the expandedKey 
      CC2520_ComputeAES( st.m_aA0, st.m_aCipherText );
      #endif      
...
// Output the right-concatenation c of the encrypted message Ciphertext ( = *(p_pucToEncrypt++) ^ aCipherText[i]) and the encrypted authentication tag U (= aInput[i] ^ aCipherText[i])
  ...
  __monitor void PHY_SetToIdle(void)
  {
      ...
      #endif
      #ifdef BBR2_HW
      if( g_ucModemState == CC2520_MODEM_STATE_POWERDOWN )
      #endif
      {
          g_ucXVRStatus = 0xFF;           // 
          PHY_Disable_Request(PHY_IDLE);
...
{  
  ...
  while(!ucLastBlock)
  {
    ...
    #endif
    #ifdef BBR2_HW
    CC2520_SetKeySA( st.m_aucOutput );
    #endif
    ...
    #ifdef BBR2_HW
    // encrypt the block using the expandedKey    
    CC2520_ComputeAES(st.m_aucInput, st.m_aucOutput);
    #endif

                                        ...
                                        const uint8 *       p_pSrcAddr128 )
{
  ...
  {
  case SRVC_READ_RSP : pNext = ASLSRVC_getReadRspObj(   p_pInBuffer, p_unInBufLen, &p_pGenericObj->m_stSRVC.m_stReadRsp ); break;
  case SRVC_WRITE_RSP: pNext = ASLSRVC_getWriteRspObj(  p_pInBuffer, p_unInBufLen, &p_pGenericObj->m_stSRVC.m_stWriteRsp ); break;
  case SRVC_EXEC_RSP : pNext = ASLSRVC_getExecuteRspObj(p_pInBuffer, p_unInBufLen, &p_pGenericObj->m_stSRVC.m_stExecRsp ); break;
  case SRVC_READ_REQ : pNext = ASLSRVC_getReadReqObj(   p_pInBuffer, p_unInBufLen, &p_pGenericObj->m_stSRVC.m_stReadReq ); break;
  case SRVC_WRITE_REQ: pNext = ASLSRVC_getWriteReqObj(  p_pInBuffer, p_unInBufLen, &p_pGenericObj->m_stSRVC.m_stWriteReq ); break;
  case SRVC_EXEC_REQ : pNext = ASLSRVC_getExecuteReqObj(p_pInBuffer, p_unInBufLen, &p_pGenericObj->m_stSRVC.m_stExecReq ); break;
...

{
...
#if ( _UAP_TYPE != 0 )  
  pNext->m_stInfo.m_stTx.m_ucRetryNo    = (UAP_DMAP_ID == p_ucSrcTSAPID ? g_ucDmapMaxRetry : g_ucMaxUAPRetries);
#else
  pNext->m_stInfo.m_stTx.m_ucRetryNo    = g_ucDmapMaxRetry;
#endif  
  pNext->m_stInfo.m_stTx.m_ucReqID      = g_ucReqID;
  pNext->m_stInfo.m_stTx.m_unAppHandle  = p_unAppHandler; // GET_NEW_APP_HANDLE();
  ...
  if( !p_unBinSize ) 
  {
      ...
      case SRVC_READ_REQ:   pBuf = (uint8*)ASLSRVC_setReadReqObj( (READ_REQ_SRVC*)p_pObj, unOutBufLen, pBuf); break;
      case SRVC_WRITE_REQ:  pBuf = (uint8*)ASLSRVC_setWriteReqObj( (WRITE_REQ_SRVC*)p_pObj, unOutBufLen, pBuf);break;
      case SRVC_EXEC_REQ:   pBuf = (uint8*)ASLSRVC_setExecuteReqObj( (EXEC_REQ_SRVC*)p_pObj, unOutBufLen, pBuf); break;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// @author NIVIS LLC, Eduard Erdei
/// @brief  Returns the last generated request ID
/// @return the last reqID
/// @remarks
///      Access level: user level
////////////////////////////////////////////////////////////////////////////////////////////////////
uint8 ASLSRVC_GetLastReqID( void )
{
    return g_ucReqID-1;
}
                                  ...
                                  uint8 *  p_pOutBuf)
{
  *p_pOutBuf = (p_ucSrvPrimitive << 7) | p_ucSrvType;
  ...
  // determine object identifier addressing mode; 
  // inferred mode not used in this stage (will be used only at concatenation)
...
const uint8 * ASLSRVC_ExtractAPDUHeader(  const uint8 * p_pData,
                                          ...
                                          uint16 * p_pDstOID)
{
  static uint16 unLastSrcOID;
  static uint16 unLastDstOID;

  uint8 ucOIDMode = ( *(p_pData++) & 0x60 ) >> 5;
  ...
  switch ( ucOIDMode )
  {
      ...
      case OID_INFERRED:
          *p_pSrcOID = unLastSrcOID;
          *p_pDstOID = unLastDstOID;
          break;

      default:
          break;
  }
  unLastSrcOID = *p_pSrcOID;
  unLastDstOID = *p_pDstOID;

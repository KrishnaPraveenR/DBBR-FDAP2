...
#if defined(BACKBONE_SUPPORT) && !defined(at91sam7x512)
  #include "uart_link.h"
#endif
...
#include "mlsm.h"
      ...
      #include "../UnitTest/ASL/unit_test_ASL.h"
  #endif
#endif
...
static void ASLDE_retryMessage(ASL_QUEUE_HDR* p_pMsgHdr);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// @author NIVIS LLC, Eduard Erdei
/// @brief  Removes an APDU from the global common RX/TX APDU buffer
/// @param  p_pAPDU - pointer to the APDU
/// @return none
/// @remarks
///      Access level: user level
////////////////////////////////////////////////////////////////////////////////////////////////////
void ASLDE_DeleteAPDU(uint8 *  p_pAPDU)
{
  ...
  uint16 unLen = ((ASL_QUEUE_HDR*)p_pAPDU)->m_unLen;

  g_pAslMsgQEnd -= unLen;
  
  memmove( p_pAPDU,
           p_pAPDU + unLen,
           (uint16)(g_pAslMsgQEnd - p_pAPDU) );
...
void ASLDE_PerformOneSecondOperations(void)
{
    ...
    while (pData < g_pAslMsgQEnd)
    {
        if ( ((ASL_QUEUE_HDR*)pData)->m_unTimeout )
        {
            ((ASL_QUEUE_HDR*)pData)->m_unTimeout--;
        }
        else
        {
            if ( ((ASL_QUEUE_HDR*)pData)->m_ucDirection == RX_APDU )
            {   // delete; this APDU was not handled by any app process
                ((ASL_QUEUE_HDR*)pData)->m_ucStatus = APDU_READY_TO_CLEAR;
            }
            else
            {   // call APDU retry function
                ASLDE_retryMessage((ASL_QUEUE_HDR*)pData);
            }
        }
        pData += ((ASL_QUEUE_HDR*)pData)->m_unLen;
    }
...
///      Access level: user level
////////////////////////////////////////////////////////////////////////////////////////////////////
void ASLDE_retryMessage(ASL_QUEUE_HDR* p_pMsgHdr)
{
  if (p_pMsgHdr->m_stInfo.m_stTx.m_ucRetryNo)
  {
      p_pMsgHdr->m_stInfo.m_stTx.m_ucRetryNo--;
      p_pMsgHdr->m_unTimeout = p_pMsgHdr->m_stInfo.m_stTx.m_unDefTimeout;
      p_pMsgHdr->m_ucStatus  = APDU_READY_TO_SEND;
  }
  else
  {   // all retries have failed; delete the APDU;
      p_pMsgHdr->m_ucStatus = APDU_READY_TO_CLEAR;
      // notify other UAPs about this failure;
      UAP_DataConfirm( p_pMsgHdr->m_stInfo.m_stTx.m_unAppHandle, SFC_TIMEOUT);
  }
}
...
uint8 * ASLDE_concatenateAPDUs( uint16      p_unContractID,
                                ...
                                uint8       p_ucHandle  )
{
  uint16 unNewSrcObjID = 0;
  uint16 unNewDstObjID = 0;
  uint16 unOldSrcObjID = 0;
  uint16 unOldDstObjID = 0;
            ...
            && pHdr->m_ucStatus == APDU_READY_TO_SEND )
      {
          if ( p_unContractID == pHdr->m_stInfo.m_stTx.m_stDest.m_unContractId )
          {
              if ( pHdr->m_unAPDULen <= (p_pOutBufEnd - p_pOutBuf) )
              {
                  ...
                  // now try to use inferred mode compression for APDU header                  
                  const uint8 * pTmp = pData+sizeof(ASL_QUEUE_HDR);
                  
                  pTmp = ASLSRVC_ExtractAPDUHeader( pTmp, &unNewSrcObjID, &unNewDstObjID );
  
                  if( unNewSrcObjID == unOldSrcObjID && unNewDstObjID == unOldDstObjID )
                  {
                      // objectIDs are the same; use inferred object addressing mode                     
                      *(p_pOutBuf++) = *(pData+sizeof(ASL_QUEUE_HDR)) | 0x60; // force bit5 and bit6 to 1
                      
                      uint8 ucAPDUHeaderSize = pTmp - (pData+sizeof(ASL_QUEUE_HDR));
                      
                      memcpy( p_pOutBuf, 
                              pTmp, 
                              pHdr->m_unAPDULen - ucAPDUHeaderSize );
                      
                      p_pOutBuf += pHdr->m_unAPDULen - ucAPDUHeaderSize;
                  }
                  else
                  {
                      memcpy( p_pOutBuf,
                  ...
                  unOldSrcObjID = unNewSrcObjID;
                  unOldDstObjID = unNewDstObjID;
                            ...
                            uint8         p_ucTransportTime)
{
  ...
  g_ulMsgTaiMicroSec = GetMSecMessageDelay(g_ulMsgTaiSec, g_ulMsgTaiMicroSec);
#endif
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
/// @author NIVIS LLC, Eduard Erdei
/// @brief  Accepts a route lookup request from the backbone router and contructs the APDU
/// @param  p_pucData - pointer to the request data
/// @param  p_ucLen   - length of request data
/// @return service feedback code
/// @remarks
///      Access level: user level\n
///      Context: Called by UART_LINK_Task of the transceiver
////////////////////////////////////////////////////////////////////////////////////////////////////
#if (DEVICE_TYPE == DEV_TYPE_AP91SAM7X512)
uint8 BBRME_RequestRoute(uint8* p_pucData, uint8 p_ucLen)
{
  if ((p_ucLen == 6) || (p_ucLen == 20))
  {
      EXEC_REQ_SRVC stExecReq;
      uint8         aucReqParams[16];
      stExecReq.m_unDstOID = ((uint16)(p_pucData[0] << 8) | p_pucData[1]);
      stExecReq.m_unSrcOID = ((uint16)(p_pucData[0] << 8) | p_pucData[1]);
      stExecReq.m_ucMethID = (((uint16)(p_pucData[2])) << 8) | p_pucData[3];
      stExecReq.p_pReqData = aucReqParams;

      if (stExecReq.m_ucMethID == 1)
      {
      ...
      // insert short addres or 16octet address
      memcpy(stExecReq.p_pReqData,
             p_pucData + 4,
             stExecReq.m_unLen);
